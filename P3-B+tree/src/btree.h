...
*/
	struct IndexMetaInfo{
		...
		/**
         * Record the root page is leaf or not
         */
		bool root_is_leaf;

		/**
         * The level of the root, the  leaf node are in the level 0
         */
		int root_level;
	};
...
*/
	...
	private:
		void insert(int level, Page* currpage, RIDKeyPair<int> new_pair, int *new_child_entry, PageId *new_page_id, BufMgr* bufMgr);

		void split_leaf(LeafNodeInt *curr,int *new_child_key, PageId *new_page_id, RIDKeyPair<int> new_pair);
		void split_nonleaf(NonLeafNodeInt *curr,int *new_child_key, PageId *new_page_id);
		void scan(Page* curr, int level, PageId pagenum);
		/**
	...
	public:
		...
		/**
     * @brief Insert
     * @param level the level of the leaf node is 0 (if leaf node)
     * @param currpage
     * @param new_pair
     * @param new_child_entry
     * @param bufMgr (pin and unpin)
     */
